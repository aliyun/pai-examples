...
class SftArguments:
    ...
    tuner_bankend: str = field(default="swift", metadata={"choices": ["swift", "peft"]})
    ...
    dtype: str = field(default="bf16", metadata={"choices": ["bf16", "fp16", "fp32"]})
    ...
    use_flash_attn: Optional[bool] = field(
        default=None,
        metadata={
            "help": "This parameter is used only when model_type.startswith('qwen')"
        },
    )
    ...
    def init_argument(self):
        ...
        if self.push_to_hub:
            ...
            if self.hub_token is not None:
                ...
                assert (
                    ModelScopeConfig.get_token() is not None
                ), "Please enter hub_token"
...
DTYPE_MAPPING = {"fp16": torch.float16, "bf16": torch.bfloat16, "fp32": torch.float32}


def select_dtype(args: Union[SftArguments, InferArguments]) -> Tuple[Dtype, bool, bool]:
    dtype = args.dtype
...
def select_bnb(args: Union[SftArguments, InferArguments]) -> Tuple[Dtype, bool, bool]:
