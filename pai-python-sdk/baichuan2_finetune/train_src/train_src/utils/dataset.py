...
def _preprocess_alpaca_dataset(
    dataset: HfDataset, preprocess_input: Optional[Callable[[str], str]] = None
) -> HfDataset:
    query: List[str] = []
    response = []
    for d in dataset:
...
def get_alpaca_gpt4_en_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "AI-ModelScope/alpaca-gpt4-data-en", split="train"
    ).to_hf_dataset()
...
def get_advertise_gen_dataset() -> Tuple[HfDataset, HfDataset]:
    dataset_train: HfDataset = MsDataset.load(
        "lvjianjin/AdvertiseGen", split="train"
    ).to_hf_dataset()
    dataset_val: HfDataset = MsDataset.load(
        "lvjianjin/AdvertiseGen", split="validation"
    ).to_hf_dataset()
    return [
...
def get_alpaca_gpt4_zh_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "AI-ModelScope/alpaca-gpt4-data-zh", split="train"
    ).to_hf_dataset()
...
def get_finance_en_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "wyj123456/finance_en", split="train"
    ).to_hf_dataset()
    return _preprocess_alpaca_dataset(dataset)
...
def _get_multi_alpaca(subset_name: str) -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "damo/nlp_polylm_multialpaca_sft", subset_name=subset_name, split="train"
    ).to_hf_dataset()
    return _preprocess_alpaca_dataset(dataset)
...
def get_code_alpaca_en_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "wyj123456/code_alpaca_en", split="train"
    ).to_hf_dataset()
    return _preprocess_alpaca_dataset(dataset)
...
def get_instinwild_zh_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "wyj123456/instinwild", subset_name="default", split="train"
    ).to_hf_dataset()
    return _preprocess_alpaca_dataset(dataset)
...
def get_instinwild_en_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "wyj123456/instinwild", subset_name="subset", split="train"
    ).to_hf_dataset()
    return _preprocess_alpaca_dataset(dataset)


def get_cot_en_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load("YorickHe/CoT", split="train").to_hf_dataset()
    return _preprocess_alpaca_dataset(dataset)


def get_cot_zh_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "YorickHe/CoT_zh", split="train"
    ).to_hf_dataset()
    return _preprocess_alpaca_dataset(dataset)


def _preprocess_mutimodal_dataset(
    dataset: HfDataset, prompt: str, image_key: str, response_key: str
) -> HfDataset:
...
def get_coco_en_dataset() -> Tuple[HfDataset, HfDataset]:
    ...
    return tuple(
        _preprocess_mutimodal_dataset(
            dataset, "please describe the image", "image", "caption"
        )
...
def _filter_agent_dataset(
    dataset: List[Dict[str, Any]], use_mini: bool
) -> List[Dict[str, Any]]:
    if use_mini:
...
def _preprocess_agent_dataset(dataset: List[Dict[str, str]]) -> HfDataset:
    ...
    dataset = HfDataset.from_dict(
        {"system": system, "query": query, "response": response, "history": history}
    )
    return dataset


def get_damo_agent_zh_dataset(use_mini: bool = False) -> Tuple[HfDataset, HfDataset]:
...
def _preprocess_firefly(
    dataset: List[Dict[str, str]], kind_list: List[str]
) -> HfDataset:
...
def get_poetry_zh_dataset() -> Tuple[HfDataset, HfDataset]:
    ...
    for dataset in (train_dataset, val_dataset):
        dataset_list.append(
            HfDataset.from_dict(
                {"query": ["写诗"] * len(dataset), "response": dataset["text1"]}
            )
        )
    return tuple(dataset_list)
...
def get_instruct_en_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "wyj123456/instruct", split="train"
    ).to_hf_dataset()
    query = []
    response = []
    for d in tqdm(dataset):
...
def get_gpt4all_en_dataset() -> HfDataset:
    dataset: HfDataset = MsDataset.load(
        "wyj123456/GPT4all", split="train"
    ).to_hf_dataset()
    return _preprocess_alpaca_dataset(dataset)


def _preprocess_cls_dataset(
    dataset: HfDataset, cls_mapping: List[str], task: str, pair_seq: bool
) -> HfDataset:
...
Output: """
    ...
    for d in tqdm(dataset):
        if d["label"] is None:
            continue
        if pair_seq:
            q = prompt.format(sentence1=d["sentence1"], sentence2=d["sentence2"])
        else:
...
def get_cmnli_zh_dataset() -> Tuple[HfDataset, HfDataset]:
    ...
    return tuple(
        _preprocess_cls_dataset(
            dataset, cls_mapping, "Natural Language Inference", True
        )
        for dataset in (train_dataset, val_dataset)
    )
...
def get_jd_zh_dataset() -> Tuple[HfDataset, HfDataset]:
    ...
    return tuple(
        _preprocess_cls_dataset(dataset, cls_mapping, "Sentiment Classification", False)
        for dataset in (train_dataset, val_dataset)
    )
...
def get_medical_dataset(
    subset_name: str, train_dataset_sample: int = -1
) -> Tuple[HfDataset, HfDataset]:
    """
    mode: Literal['en', zh]
    """
    dataset_dict = MsDataset.load("huangjintao/medical_zh", subset_name=subset_name)
    train_dataset: HfDataset = concatenate_datasets(
        [
            dataset_dict["train"].to_hf_dataset(),
...
def _preprocess_sharegpt(dataset: HfDataset) -> HfDataset:
    ...
    for d in tqdm(dataset):
        ...
        history.append(h)
    return HfDataset.from_dict(
        {"query": query, "response": response, "history": history}
    )
...
def get_sharegpt_dataset(subset_name_list: List[str]) -> HfDataset:
    ...
    for subset_name in subset_name_list:
        dataset = MsDataset.load(
            "huangjintao/sharegpt", subset_name=subset_name, split="train"
        ).to_hf_dataset()
        dataset_list.append(dataset)
    dataset = concatenate_datasets(dataset_list)
...
def get_cls_fudan_news_zh() -> HfDataset:
    ...
    return HfDataset.from_dict(
        {"query": dataset["prompt"], "response": dataset["answer"]}
    )
...
DATASET_MAPPING = {
    ...
    "medical-en": partial(get_medical_dataset, subset_name="en"),
    "medical-zh": partial(get_medical_dataset, subset_name="zh"),
    "medical-mini-zh": partial(
        get_medical_dataset, subset_name="zh", train_dataset_sample=100000
    ),
    "code-python-zh": get_code_python_zh_dataset,
    # multi-round chat
    ...
    "sharegpt-en": get_sharegpt_all_en_dataset,
    "sharegpt-zh": get_sharegpt_all_zh_dataset,
    # nlp text-generation
    ...
    "cls-fudan-news-zh": get_cls_fudan_news_zh,  # seqgpt-560m
    "ner-jave-zh": get_ner_jave_zh,  # seqgpt-560m
}


def get_dataset(
    dataset_name_list: List[str],
    dataset_test_ratio: float = 0.0,
