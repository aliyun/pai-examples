...
MODEL_ID_OR_PATH = os.environ.get(
    "PAI_INPUT_MODEL", "damo/cv_vit-base_image-classification_ImageNet-labels"
)
...
def save_model():
    best_ckpt_pattern = re.compile(pattern=r"^best_accuracy_top-1_epoch_\d+.pth$")
    ...
    if f_name:
        ...
        if os.path.isfile(os.path.join(WORK_DIR, last_ckpt_file)):
            ...
            print(
                "Not found latest checkpoint: {}.".format(
                    os.path.join(WORK_DIR, last_ckpt_file)
                )
            )
    # 模型配置信息
    shutil.copyfile(
...
def train():
    ms_train_dataset = MsDataset.load(
        "flowers14", namespace="tany0699", subset_name="default", split="train"
    )  # 加载训练集

    ms_val_dataset = MsDataset.load(
        "flowers14", namespace="tany0699", subset_name="default", split="validation"
    )  # 加载验证集
